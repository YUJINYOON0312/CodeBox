...
public class MyPageServiceProcess implements MyPageService {
	...
	public void info(long mno, Model model) {
		...
		// 로그인한 회원의 주소정보
		model.addAttribute("mpageaddr", aRepo.findByMember_mno(mno).map(AddressDetailDTO::new).orElseThrow());
	}

	@Transactional
	@Override // 급여관리리스트 회원정보
	public void list(long mno, Model model) {
		List<SalaryListDTO> result = mRepo.findAll().stream().map(SalaryListDTO::new).collect(Collectors.toList());
	...
	public void salaryInfo(long mno, Model model) {
		...
		List<DepartmentDTO> result = dRepo.findAllByDepth(3).stream().map(DepartmentDTO::new).collect(Collectors.toList());
		model.addAttribute("department", result);

		List<PositionDTO> aaa = pRepo.findAll().stream().map(PositionDTO::new).collect(Collectors.toList());
	...
	public void update(long mno, long pno, long dno) {
		...
		for (DepartmentMemberEntity d : temp) System.out.println(">>>>>>>>>>>>>>>>>>>>>>" + temp);
