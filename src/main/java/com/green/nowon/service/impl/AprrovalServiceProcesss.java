...
public class AprrovalServiceProcesss implements ApprovalService {
	...
	public void save(ApprovalSaveDTO dto) {
		repo.save(ApprovalEntity.builder()
				...
				.status("결재 대기중")
				.adno(null)
				.mno(MemberEntity.builder().mno(dto.getMno()).build())
				.build());
	...
	public void list(Model model, int page) {
		...
		model.addAttribute("list", result.stream().map(ApprovalListDTO::new).collect(Collectors.toList()));
	}

	@Override
	public void detail(long ano, Model model) {
		model.addAttribute("list", repo.findById(ano).map(ApprovalListDTO::new).orElseThrow());
	...
	public void save(AttendanceSaveDTO dto) {
		repo.save(ApprovalEntity.builder()
				...
				.title(dto.getAdtype())
				.status("결재 대기중")
				.adno(AttendanceEntity.builder().adtype(dto.getAdtype()).sdate(dto.getSdate()).edate(dto.getEdate()).build())
				.mno(MemberEntity.builder().mno(dto.getMno()).build())
