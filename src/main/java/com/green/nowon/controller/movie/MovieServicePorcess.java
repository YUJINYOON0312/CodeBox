...
import com.green.nowon.controller.movie.naver.NaverMovieResponse;
import com.green.nowon.controller.openapi.OpenApiUtil;
...
public class MovieServicePorcess implements MovieService {
	...
	public List<Item> dailyBoxOfficeList(String targetDt) {
		...
		return response.getBoxOfficeResult()
				...
				.map(i -> {
					String pubDt = searchMovieInfo(i.getMovieCd());
					String imgurl = naverApiMovie(i.getMovieNm(), pubDt);
					return i.imgUrl(imgurl);
				})
				.collect(Collectors.toList());
	}

	private String searchMovieInfo(String movieCd) {
		try {
			String result = kobis.getMovieInfo(true, movieCd);
			// "prdtYear":"2022";
			String target = "\"prdtYear\"";
			int pos = result.indexOf(target);
			String str = result.substring(pos, pos + 17).replace("\"", "");// "제거
			// System.out.println(">>"+str);
			System.out.println(str.substring(str.length() - 4));
			// System.out.println( result);
			return str.substring(str.length() - 4);// 제작연도 리턴
		} catch (Exception e) {
	...
	private String naverApiMovie(String query, String pubDt) {
